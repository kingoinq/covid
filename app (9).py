...
def finding_box(stat, stat_label, text):
    st.markdown(f"""
    <div style="background:#FEF2F2;border-left:4px solid {CRITICAL};
                padding:20px 18px;border-radius:0 8px 8px 0;margin:16px 0 10px 0;">
      <p style="color:{CRITICAL};font-size:0.68rem;font-weight:700;text-transform:uppercase;
                letter-spacing:1.1px;margin:0 0 10px 0;font-family:'Times New Roman',serif;">
        What the data shows</p>
      <p style="color:{PRIMARY};font-size:2.4rem;font-weight:900;margin:0;line-height:1;
                font-family:'Times New Roman',serif;">{stat}</p>
      <p style="color:{CRITICAL};font-size:0.75rem;font-weight:700;margin:4px 0 12px 0;
                font-family:'Times New Roman',serif;">{stat_label}</p>
      <p style="color:{PRIMARY};font-size:0.88rem;line-height:1.55;margin:0;
                font-family:'Times New Roman',serif;">{text}</p>
    </div>""", unsafe_allow_html=True)

def implication_box(stat, stat_label, text):
    st.markdown(f"""
    <div style="background:#F0FDF4;border-left:4px solid {SUCCESS};
                padding:20px 18px;border-radius:0 8px 8px 0;margin:10px 0 0 0;">
      <p style="color:{SUCCESS};font-size:0.68rem;font-weight:700;text-transform:uppercase;
                letter-spacing:1.1px;margin:0 0 10px 0;font-family:'Times New Roman',serif;">
        Why this matters</p>
      <p style="color:{PRIMARY};font-size:2.4rem;font-weight:900;margin:0;line-height:1;
                font-family:'Times New Roman',serif;">{stat}</p>
      <p style="color:{SUCCESS};font-size:0.75rem;font-weight:700;margin:4px 0 12px 0;
                font-family:'Times New Roman',serif;">{stat_label}</p>
      <p style="color:{PRIMARY};font-size:0.88rem;line-height:1.55;margin:0;
                font-family:'Times New Roman',serif;">{text}</p>
    </div>""", unsafe_allow_html=True)
...
with col_right:
    finding_box("13.1%", "Mexico's diabetes rate",
        "The highest in this group — nearly 1 in 7 adults already at serious risk before COVID arrived.")
    implication_box("1 in 7", "adults with diabetes",
        "Going into a pandemic with this many high-risk people, and too few hospitals to treat them, was a dangerous combination.")
...
with col_right2:
    finding_box("1.4", "hospital beds per 1,000 people",
        "The lowest in the group. When cases surged, the system had almost no room left to absorb the pressure.")
    implication_box("Last", "in hospital capacity among peers",
        "Hospitals do not appear overnight. The beds that were missing during COVID had been missing for years.")
...
with col_right3:
    finding_box("Top 3", "highest death rates in Latin America",
        "Mexico's death curve climbed faster and stayed higher than Chile and Colombia — countries at the same income level.")
    implication_box("Not the virus", "— the system behind it",
        "If all countries suffered equally we could blame the virus. They did not. The gap in outcomes was within human control.")
...
with col_right4:
    finding_box(cfr_val, "case fatality rate in Mexico",
        "More people died per confirmed case than in most peer countries — because far fewer were tested and many arrived at hospitals too late.")
    implication_box("Caught too late", "patients arriving in crisis",
        "A high death rate among confirmed cases means the system only sees people when they are already critical. Earlier testing changes that.")
...
with col_right5:
    finding_box("70%", "Chile's coverage when Mexico reached 30%",
        "Mexico's campaign accelerated only after its two deadliest waves had already ended. The protection came after the damage.")
    implication_box("Timing", "matters as much as coverage",
        "Getting vaccinated early is what saves lives — not just eventually. Chile proved a Latin American country could move fast.")
...
with col_right6:
    finding_box(ratio_val, "times more deaths than Chile per million",
        "Mexico and Chile sit at nearly the same income level. One was far better prepared. The difference shows in the numbers.")
    implication_box("Preparation", "— not money — is the variable",
        "This is the most important chart in the dashboard. Mexico did not lose more people because it was poor. It lost more because it was not ready.")
